...
void Fit(std::string inName){
	...
	string name = string(inName.c_str()).substr(string(inName.c_str()).find_last_of("/") + 1);
	size_t dot_position = name.find(".");
    string date_name = "h" + name.substr(0, dot_position);
	string date = name.substr(0, dot_position);


	TH1D *h1 = new TH1D(date_name.c_str(), date.c_str(), 500, -50, 200); // define histogram 
	...
	h1->SetName(date_name.c_str());
	TFile* file = new TFile("Integrated.root", "UPDATE");
  	h1->Write();
  	file->Close();

	string pdf_name = date + ".pdf";
	string pdf_path = "/home/omrir52/software/PMT_Two/PMTFitting/scripts/PDF/" + pdf_name;
